...
public final class CNPinyinIndexFactory {
    ...
    static CNPinyinIndex matcherChinese(CNPinyin cnPinyin, String keyword) {
        if (keyword.length() < cnPinyin.data.chinese().length()) {
            @SuppressLint("WrongConstant") Matcher matcher = Pattern.compile(keyword, Pattern.CASE_INSENSITIVE).matcher(cnPinyin.data.chinese());
            if (matcher.find()) {
    ...
    static CNPinyinIndex matcherFirst(CNPinyin cnPinyin, String keyword) {
        if (keyword.length() <= cnPinyin.pinyins.length) {
            @SuppressLint("WrongConstant") Matcher matcher = Pattern.compile(keyword, Pattern.CASE_INSENSITIVE).matcher(cnPinyin.firstChars);
            if (matcher.find()) {
                return new CNPinyinIndex(cnPinyin, matcher.start(), matcher.end());
            }
        }
    ...
    static CNPinyinIndex matchersPinyins(CNPinyin cnPinyin, String keyword) {
        ...
        for (int i = 0; i < cnPinyin.pinyins.length; i++) {
            ...
            if (pat.length() >= keyword.length()) {//首个位置索引
                @SuppressLint("WrongConstant") Matcher matcher = Pattern.compile(keyword, Pattern.CASE_INSENSITIVE).matcher(pat);
                ...
                @SuppressLint("WrongConstant") Matcher matcher = Pattern.compile(pat, Pattern.CASE_INSENSITIVE).matcher(keyword);
    ...
    private static int end(String[] pinyinGroup, String pattern, int index) {
        if (index < pinyinGroup.length) {
            ...
            if (pinyin.length() >= pattern.length()) {//首个位置索引
                @SuppressLint("WrongConstant") Matcher matcher = Pattern.compile(pattern, Pattern.CASE_INSENSITIVE).matcher(pinyin);
                if (matcher.find() && matcher.start() == 0) {
                    return index + 1;
                }
            } else {
                @SuppressLint("WrongConstant") Matcher matcher = Pattern.compile(pinyin, Pattern.CASE_INSENSITIVE).matcher(pattern);
                if (matcher.find() && matcher.start() == 0) {//全拼匹配第一个必须在0位置
                    String left = matcher.replaceFirst("");
